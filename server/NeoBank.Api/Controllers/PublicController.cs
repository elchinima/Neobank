...
public class PublicController : ControllerBase
{
    ...
    public PublicController(IApplicationDbContext context)
    {
        _context = context;
    }

    // ─────────────────────────────────────────────
    // PUBLIC CONTENT  (бывший PublicContentController)
    // ─────────────────────────────────────────────
    ...
    public async Task<IActionResult> GetPublicContent()
    {
        ...
        var footerContacts  = footerSettings.Where(f => f.Category == "Contact").Select(f => new { contactKey = f.Key, label = f.Key, value = f.Value, url = f.Url });
        var footerSocials   = footerSettings.Where(f => f.Category == "Social").Select(f  => new { section = "social", label = f.Key, url = f.Url, isExternal = true });
        var footerDocuments = footerSettings.Where(f => f.Category == "Document").Select(f => new { docKey = f.Key, url = f.Url });
    ...
    public async Task<IActionResult> GetPublicCashbacks()
    {
        var categories = await _context.CashbackCategories
            .Select(c => new { c.Id, c.TitleEn, c.TitleRu, c.TitleAz, c.TextEn, c.TextRu, c.TextAz, c.Rate, c.Variant })
            .ToListAsync();
    ...
    public async Task<IActionResult> RequestNewsletterSubscription([FromBody] NewsletterRequestDto dto, [FromServices] NeoBank.Application.Interfaces.IEmailService emailService)
    {
        ...
        var user = await _context.Users.Include(u => u.Session).FirstOrDefaultAsync(u => u.Email.ToLower() == dto.Email.ToLower());
        ...
        _context.EmailVerificationCodes.Add(new NeoBank.Core.Entities.EmailVerificationCode
        {
            UserId    = user.Id,
            Code      = code,
    ...
    public async Task<IActionResult> VerifyNewsletterSubscription([FromBody] NewsletterVerifyDto dto)
    {
        ...
        if (user.Session != null) user.Session.IsSubscribedToNewsletter = true;
        await _context.SaveChangesAsync();
    ...
    public async Task<IActionResult> UnsubscribeNewsletter([FromBody] NewsletterRequestDto dto)
    {
        ...
        if (user.Session != null) user.Session.IsSubscribedToNewsletter = false;
        await _context.SaveChangesAsync();
